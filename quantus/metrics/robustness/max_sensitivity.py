...
class MaxSensitivity(PerturbationMetric):
    ...
    def custom_preprocess(
        ...
        perturbed_samples = np.zeros((x_batch.shape[0], self.nr_samples, *model.shape_input(x_batch[0], x_batch[0].shape, channel_first=True).shape),
                                     dtype=float)
        ...
        a_perturbed_all = self.explain_func(
            ...
            inputs=perturbed_samples.reshape(x_batch.shape[0]*self.nr_samples, *model.shape_input(x_batch[0], x_batch[0].shape, channel_first=True).shape),
            targets=np.tile(y_batch, self.nr_samples),
            **self.explain_func_kwargs,
        )

        custom_preprocess_batch = a_perturbed_all.reshape((x_batch.shape[0], self.nr_samples, *model.shape_input(x_batch[0], x_batch[0].shape, channel_first=True).shape))
